...
export const HistoryList: React.FC<HistoryListProps> = ({
  ...
  return (
    <div className="space-y-5 max-w-4xl mx-auto">
      ...
      {notice && (
        <div
          className={`p-3.5 rounded-2xl flex items-center justify-between text-xs font-bold border-2 border-playful-dark dark:border-slate-700 shadow-pop-sm dark:shadow-[2px_2px_0px_#010409] animate-in fade-in transition-all ${
            notice.type === "warning"
              ? "bg-playful-amber dark:bg-amber-400 text-playful-dark"
              : notice.type === "info"
                ? "bg-playful-mint dark:bg-emerald-400 text-playful-dark"
                : "bg-playful-pink dark:bg-pink-400 text-playful-dark"
          }`}
        >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {/* Mode filters */}
          <div className="flex items-center bg-white dark:bg-[#21262d] p-1 rounded-2xl border-2 border-playful-dark dark:border-slate-700 shadow-pop-sm dark:shadow-[2px_2px_0px_#010409] text-xs">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-11 pr-4 py-2.5 bg-white dark:bg-[#161b22] border-2 border-playful-dark dark:border-slate-700 rounded-2xl text-xs sm:text-sm text-playful-dark dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 font-medium shadow-pop-sm dark:shadow-[2px_2px_0px_#010409] focus:outline-none focus:border-playful-violet dark:focus:border-violet-500 transition-colors"
        />
      </div>
      ...
      <div className="space-y-3">
        {filtered.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`p-4 rounded-2xl border-2 border-playful-dark dark:border-slate-700 transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${
                isMissing
                  ? "bg-amber-50/80 dark:bg-amber-950/20 shadow-pop-amber dark:border-amber-800"
                  : "bg-white dark:bg-[#161b22] shadow-pop dark:shadow-[3px_3px_0px_#010409] hover:-translate-y-0.5"
              }`}
            >
              <div className="flex items-center space-x-3.5 min-w-0 flex-1">
                {/* Thumbnail */}
                <div className="relative w-16 h-12 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-900 shrink-0 border-2 border-playful-dark dark:border-slate-700 shadow-pop-sm dark:shadow-[2px_2px_0px_#010409]">
                  {item.thumbnail ? (
                    <img
                      src={item.thumbnail}
                      alt={item.title}
                  ...
                  <div className="absolute bottom-0.5 right-0.5 px-1 py-0.2 rounded bg-playful-dark text-[8px] font-extrabold uppercase text-white border border-white dark:border-slate-700">
                    {item.mode}
                  </div>
                </div>
                ...
                <div className="min-w-0 flex-1 space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:gap-2.5 text-xs text-slate-600 dark:text-slate-400">
                    <span className="px-2 py-0.5 rounded-lg bg-playful-violet/10 dark:bg-violet-950/50 text-playful-violet dark:text-violet-300 font-mono font-bold text-[10px] border border-playful-dark dark:border-slate-700">
              ...
              <div className="flex items-center space-x-2 self-end sm:self-center shrink-0">
                <button
                  ...
                  className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border-2 border-playful-dark dark:border-slate-700 transition-all ${
                    isMissing
                      ? "bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 hover:text-playful-dark dark:hover:text-slate-300"
                ...
                <button
                  ...
                  className="p-2 bg-white dark:bg-[#21262d] hover:bg-slate-50 dark:hover:bg-slate-700 text-playful-dark dark:text-slate-200 rounded-xl border-2 border-playful-dark dark:border-slate-700 shadow-pop-sm dark:shadow-[2px_2px_0px_#010409] hover:-translate-y-0.5 active:translate-x-0.5 active:translate-y-0.5 transition-all"
                  title={
                    isMissing
